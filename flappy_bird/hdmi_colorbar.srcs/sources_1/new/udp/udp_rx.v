...
module udp_rx(
    ...
    output  reg  [3:0]   rec_data    ,    //以太网接收的数据
...
always @(posedge clk or negedge rst_n) begin
    if(!rst_n) begin
        ...
        begin
            wait_count <= (wait_count == 0)? wait_count : wait_count + 1;
            rec_data <= (wait_count == 0)?4'b0000:rec_data;
        end
        ...
        case(next_state)
            ...
            st_rx_data : begin
                ...
                if(gmii_rx_dv) begin
                    ...
                    if(gmii_rxd == 8'h77) begin
                        rec_data <= 4'b0001;
                        wait_count <= 1;
                        rec_en <= 1'b1;
                        end
                    else if(gmii_rxd == 8'h61) begin
                        rec_data <= 4'b0010;
                        wait_count <= 1;
                        rec_en <= 1'b1;
                        end
                    else if(gmii_rxd == 8'h73) begin
                        rec_data <= 4'b0100;
                        wait_count <= 1;
                        rec_en <= 1'b1;
                        end
                    else if(gmii_rxd == 8'h64) begin
                        rec_data <= 4'b1000;
                        wait_count <= 1;
                        rec_en <= 1'b1;
                        end
...
/* ila_0 ila_inst (
    ...
    .probe0({gmii_rxd,gmii_rx_dv,error_en,skip_en,next_state,rec_en}) // input wire [99:0] probe0
); */
